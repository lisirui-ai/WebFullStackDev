...
// ---- 写法五：返回对象字面量时，需用 () 包裹，否则 {} 会被解析为函数体 ----
// 返回值（object）：包含 name 和 age 字段的对象
...
const obj = {
    ...
    normalFunc() {
        ...
        setTimeout(function() {
            ...
            // setTimeout 的回调由浏览器计时器调用，调用者不是 obj
            // 严格模式下 this → undefined；非严格模式下 this → window
            console.log(this.name); // undefined 或报错，无法访问 obj.name
        }, 1000);
    },
...
obj.normalFunc(); // 1秒后输出：undefined
obj.arrowFunc();  // 1秒后输出：Vue组件
```
...
// 为什么需要 qs？
//   Axios POST 默认将对象序列化为 JSON 字符串（Content-Type: application/json）
//   而 Django 的 request.POST 只能解析 urlencoded 格式（key=value&key2=value2）
//   使用 qs.stringify() 可将对象转为 urlencoded 字符串，Django 后端无需额外处理
...
methods: {
    ...
    submitForm() {
        ...
        //        传字符串时 Content-Type 自动设为 application/x-www-form-urlencoded
        //        传对象时 Content-Type 自动设为 application/json
        // 返回值：Promise 对象
        ...
        .then(res => {
            ...
            // 作用：将服务器返回的 token 持久化到 Vuex state，后续请求的拦截器会读取它
        ...
        // 直接传对象，Axios 自动将其序列化为 JSON 字符串
        // 请求头 Content-Type 自动设为 'application/json'
        // Django 后端需要通过 json.loads(request.body) 来解析数据
...
// 实际请求：http://localhost:8080/api/users/（发给 webpack-dev-server，同源无跨域）
// 转发路径：/api/users/ → 去掉 /api → /users/
// 后端收到：http://localhost:8000/users/（服务器间转发，无跨域限制）
```
