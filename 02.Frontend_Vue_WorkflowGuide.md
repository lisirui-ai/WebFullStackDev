...
//            render 函数比 template 字符串更高效（无需运行时编译模板）
...
// 直接导入（非懒加载）：打包时两个组件都会打进主 bundle，适合首屏就可能用到的页面
// 懒加载写法（按需分割代码）：() => import('@/pages/HomePage')
...
export default new Router({
    ...
    // routes：路由规则数组（RouteConfig[]），Vue Router 按数组顺序逐条匹配当前 URL
    // 采用"首次匹配即停止"策略，所以通配符路由（404 页）要放在数组最末尾
    routes: [
        // ── 单条 RouteConfig 对象字段说明 ──────────────────────────────────
        //   path      {string}   - URL 路径模式；支持动态段 :param、可选段 :param?、通配符 *
        //   name      {string}   - 路由命名（可选），编程式导航时可用名称代替路径
        //                          用法：this.$router.push({ name: 'detail', params: { id: 1 } })
        //   component {Component}- URL 匹配时，在最近的 <router-view> 中渲染此组件
        //   children  {Array}    - 嵌套子路由（此处未使用）
        //   meta      {Object}   - 路由元信息，可存储权限标志等自定义字段（此处未使用）
...
// Vuex 提供 Store 类和 mapState / mapGetters / mapMutations / mapActions 辅助函数
import Vuex  from 'vuex'
...
export default new Vuex.Store({
    ...
    // ── state：唯一数据源（Single Source of Truth）────────────────────────────
    // 存放所有需要跨组件共享的响应式数据，Vue 会对其进行响应式处理
    // 组件读取方式：
    //   1. 直接访问：this.$store.state.newsList
    //   2. 计算属性映射：computed: { ...mapState(['newsList', 'loading']) }
    ...
    // context 完整结构：{ commit, state, dispatch, getters, rootState, rootGetters }
    // 实际开发中常解构为 ({ commit }) => {} 只取 commit，减少代码量
    actions: {
    ...
    // 特点：结果会被 Vuex 缓存，依赖的 state 不变则不重新计算（性能优化）
    // getter 函数完整签名：(state, getters, rootState, rootGetters) => any
    // 组件访问方式：
    //   1. 直接访问：this.$store.getters.newsCount
    //   2. 映射：computed: { ...mapGetters(['newsCount']) }
    getters: {
...
export default {
    // name：组件名称字符串
    // 作用①：在 Vue DevTools 面板中显示该名称，便于调试定位
    // 作用②：递归组件时，组件在模板中引用自身的名称
    name: 'HomePage',

    // components：局部注册子组件，仅在本组件模板中生效
    // 对象键名即模板中使用的标签名（NavBar → <NavBar />，NewsCard → <NewsCard />）
    // 局部注册优于全局注册：不使用该组件的页面不会打包此代码（Tree-shaking 友好）
...
/* .news-list：新闻卡片列表容器
   display: flex   → 开启 Flex 弹性布局，子元素（NewsCard）水平排列
   flex-wrap: wrap → 超出容器宽度时自动换行（一行排不下时换到下一行）
   gap: 20px       → 子元素之间的水平和垂直间距均为 20px（比 margin 更简洁，不受外边距折叠影响） */
.news-list { display: flex; flex-wrap: wrap; gap: 20px; }
</style>
```

### 18.6 新闻卡片子组件（components/NewsCard.vue）
...
<script>
// NewsCard 是纯展示型子组件（Presentational Component / Dumb Component）：
//   - 只通过 props 接收数据，不直接访问 Vuex / 路由
//   - 只通过 $emit 向父组件传递用户交互，不执行路由跳转
//   - 这种设计使组件高度可复用：在任意父组件中只需传入 news 对象即可使用
export default {
    // name：组件名称（String）
    // ① Vue DevTools 中以此名称标识组件节点，便于调试
    // ② 组件需要递归引用自身时，在模板中使用此名称
    name: 'NewsCard',
...
### 18.7 导航栏组件（components/NavBar.vue）
...
<template>
    ...
    <nav class="navbar">
        ...
        <!-- <ul> 导航链接列表：用语义化列表标签包裹导航项，结构清晰
             margin-left: auto 将此列表推到导航栏最右侧（flex 弹性布局特性） -->
        <ul class="nav-links">
...
/* .navbar：顶部导航栏容器
   ...
   top: 0; left: 0; right: 0 → 四边贴紧视口顶部、左右撑满全屏（等价于 width: 100%，但更健壮）
   height: 60px         → 固定高度，HomePage 的 .content 设置了 margin-top: 80px 以避开遮挡
   background: #2c3e50  → 深蓝灰色背景（即 Flat UI 中的 "Wet Asphalt" 色）
   display: flex        → 开启 Flex 布局，子元素（.brand、.nav-links）水平排列
   align-items: center  → 子元素在交叉轴（垂直方向）居中对齐，文字垂直居中于导航栏
...
/* .nav-links：导航链接列表容器
   ...
   display: flex        → 子 <li> 水平排列
   gap: 20px            → 链接之间的间距，比 margin-right 更简洁（无需处理最后一项）
   margin-left: auto    → Flex 布局技巧：auto 边距会吸收父容器剩余空间，
                          将 .nav-links 整体推到导航栏最右侧（等效于 float: right） */
...
/* .nav-links a：导航链接默认样式（未激活状态）
   ...
   padding: 5px 10px            → 扩大可点击热区，提升用户点击精度（Fitts's Law） */
...
/* .nav-links a.active：当前激活路由对应的导航链接高亮样式
   ...
   color: #fff                  → 纯白色，亮度高于未激活链接，突出当前页
   border-bottom: 2px solid #3498db → 蓝色底部下划线（#3498db 即 Flat UI 的 "Peter River" 蓝），
                                       作为当前页的视觉指示器，让用户清楚所在位置 */
...
# 构建过程包含以下优化：
#   ① 代码压缩（Minify）：删除空白/注释，缩短变量名，减小文件体积
#   ② Tree-shaking：静态分析 import/export，删除未被引用的代码（如未用的工具函数）
#   ③ 代码分割（Code Splitting）：按路由/动态 import 分割为多个 chunk，
#      用户只加载当前页面需要的代码，首屏更快
#   ④ 资源 hash 命名：如 app.a1b2c3d4.js，文件内容变更则 hash 变化，
#      可设置 CDN 永久缓存（Cache-Control: max-age=31536000），内容变更时 URL 自动失效
...
# 问题：Vue Router 的 history 模式使用 HTML5 pushState API 改变 URL，
#       这些 URL 变化只在浏览器端存在，服务器上没有对应的目录或文件
#       当用户刷新 /detail/42 或直接输入该 URL 时，Nginx 会尝试找 /var/www/html/dist/detail/42
#       这个路径不存在，Nginx 返回 404 错误
# 解决：配置 try_files 将所有未匹配到实际文件的请求都返回 index.html，
...
#     location / {
#         # try_files 依次尝试：
#         #   $uri       → 精确文件（如 /static/app.js）
#         #   $uri/      → 目录下的 index.html
#         #   /index.html → 若前两者都找不到，统一返回 dist/index.html
#         # Vue Router 收到请求后，根据 URL 渲染对应页面组件
